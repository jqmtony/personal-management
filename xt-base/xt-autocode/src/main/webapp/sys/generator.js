...
step2Panel = new FDFormPanel({
    ...
    , controls: [
        new FDTextBox({
            domId: 'txt-packageName',
            name: 'packageName',
            nativeAttr: {placeholder: 'com.aa.bb'},
            width: '300px'
        }),
        new FDTextBox({
            domId: 'txt-author',
            name: 'author',
            nativeAttr: {placeholder: '必填'},
            width: '300px'
        })
        , new FDTextBox({
            domId: 'txt-charset',
            name: 'charset',
            defaultValue: 'UTF-8',
            nativeAttr: {placeholder: '输入文件编码,如:UTF-8'}
        })
    ]
});
...
function queryTable() {
    gridTable.search({dcId: dcId, tableName: $('input[name="tableName"]').val()});
}
